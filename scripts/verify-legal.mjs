#!/usr/bin/env node
/**
 * Checks that the shared legal documents in this repo still match the other repo's copy.
 *
 * `src/lib/legalDocuments.ts` and the order server's `backend/src/legal/legalDocuments.ts`
 * are the same file. Nothing about that is enforceable from CI - two repositories, two
 * runners, no shared checkout - so this is the gate, and it has to be run.
 *
 * It is modelled on `frontend/`'s `verify:lock` in the other repo: a real check that runs
 * locally, rather than a comment asking people to remember.
 *
 * Point it at the sibling checkout with EVERSWEET_APP_REPO if it is not in the usual place.
...
const MINE = join(repoRoot, "src", "lib", "legalDocuments.ts");
...
const CANDIDATES = process.env.EVERSWEET_APP_REPO
  ? [process.env.EVERSWEET_APP_REPO]
  : [
      "C:/Users/User/OneDrive/Desktop/eversweet_APP/eversweet_app",
      resolve(repoRoot, "..", "eversweet_app"),
    ];
...
for (const candidate of CANDIDATES) {
  const path = join(candidate, "backend", "src", "legal", "legalDocuments.ts");
...
if (!theirs) {
  ...
  if (process.env.CI) {
    console.error(
      "verify:legal - running in CI with no copy of the order server to compare against. " +
  ...
  console.log(
    "verify:legal - the order server's checkout is not on this machine, so there is " +
      "nothing to compare against. Set EVERSWEET_APP_REPO to check.",
  );
  process.exit(0);
}
...
console.error(
  [
    ...
    "  .prettierignore - this repo's Prettier adds semicolons and the other repo's",
    "  does not.",
    "",
  ].join("\n"),
);
