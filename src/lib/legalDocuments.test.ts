  ...
  it("leaves no empty bullets", () => {
    for (const section of document.sections) {
      for (const item of section.list ?? []) {
        expect(
          item.trim().length,
          `"${section.heading}" has a blank bullet`,
        ).toBeGreaterThan(3);
      }
    }
  });
...
describe("the claims that were wrong before", () => {
  ...
  it("ties the streak to the subscription, not to a missed month", () => {
    ...
    expect(termsText).toMatch(
      /renewal that fails and is then paid keeps your discount/i,
    );
  ...
  it("says when points expire, because they now do", () => {
    ...
    expect(termsText).toMatch(
      /a month passes without an order from you in the app/,
    );
  ...
  it("says an active, paid-up membership keeps points from expiring", () => {
    expect(termsText).toMatch(
      /membership is active and paid up, your points do not expire/,
    );
    expect(termsText).toMatch(/runs from the day it ended/);
  });

  /**
   * A membership on hold is still `isActive`, and until 2026-09-25 the order server's cart
   * and points rate asked only that - so a member whose card had stopped paying kept member
   * prices, member-only items and 1.5x points. Every benefit now pauses, and the terms must
   * say so.
   */
  it("says every member benefit pauses while a membership is on hold", () => {
    ...
    expect(termsText).toMatch(
      /While your membership is on hold, its benefits are paused/,
    );
    expect(termsText).toMatch(/member-only items are taken out of it/);
    expect(termsText).toMatch(
      /While it is on hold, your month runs from the day the unpaid renewal was due/,
    );
  });
  ...
  it("says a customer is told before their points go", () => {
    ...
    expect(privacyText).toMatch(
      /a week before your Sweet Points are due to expire/,
    );
  });
...
describe("age", () => {
  ...
  it("names no other minimum age", () => {
    for (const text of [termsText, privacyText]) {
      const ages = [
        ...text.matchAll(/\b(?:at least|under|aged) (\d+)\b/gi),
      ].map((match) => match[1]);
